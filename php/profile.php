...
<body>
      ...
      <nav>
        <div class="container nav_container">
            <a href="index.html"><h4>PedaONLINE</h4></a>
            <ul class="nav_menu">
        ...
        <div class="container categories_container">
            ...
            <div class="categories_right">
                <article class="category">
                    ...
                    <h5>Courses you are involved to: 
                    <?php

                        if($_SESSION['role'] == "Instructor") {
                            $database_query = "SELECT courseName FROM proiect_baza_de_date.instructors WHERE userID = '" . $_SESSION['userID'] . "'";
                            $result = mysqli_query($database_connection, $database_query);
                            if ($result) {
                                while ($row = mysqli_fetch_assoc($result)) {
                                    echo htmlspecialchars($row['courseName']) . ", ";
                                }
                                mysqli_free_result($result);
                            } else {
                                echo "Query error to database: " . mysqli_error($database_connection);
                            }
                        }else{
                            $database_query = "SELECT courseName FROM proiect_baza_de_date.courses WHERE userID = '" . $_SESSION['userID'] . "'";
                            $result = mysqli_query($database_connection, $database_query);
                            if ($result) {
                                while ($row = mysqli_fetch_assoc($result)) {
                                    echo htmlspecialchars($row['courseName']) . ", ";
                    ...
                    <a class="btn btn-primary" href="subscriptions.php">Upgrade subscription</a>
                    <!-- <a class="btn btn-primary" href="cancel_subscription.php">Cancel your subscription</a> -->
                    <a class="btn btn-primary" href="../images/easteregg.jpg">Cancel your subscription</a>
                </article>
